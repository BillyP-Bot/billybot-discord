...
export class StocksMethods {

	/*
		!stock [tickerSymbol]
		show the current price of the stock for the specified ticker symbol
	*/
	static async ShowPrice(msg: Message, prefix: string): Promise<void> {
		try {
			...
			try {
				const res = await StocksMethods.GetCurrentStockInfo(symbol);
				if (!res) throw `Cannot find ticker symbol: '${symbol}'!`;
	
				MessagesMethods.ReplyWithSuccessEmbed(msg, symbol, `${res.price} (${res.currency})`);
			} catch (error) {
	...
	*/
	static async Buy(msg: Message, prefix: string): Promise<void> {
		try {
			const args: string[] = msg.content.slice(prefix.length).trim().split(" ");
			...
			const currentStockInfo = await StocksMethods.GetCurrentStockInfo(symbol);
			...
			MessagesMethods.ReplyWithSuccessEmbed(msg, "Stock Purchased", `You invested ${amount} BillyBucks in '${symbol}' at ` +
	...
	*/
	static async Sell(msg: Message, prefix: string): Promise<void> {
		try {
			const args: string[] = msg.content.slice(prefix.length).trim().split(" ");
			...
			MessagesMethods.ReplyWithSuccessEmbed(msg, "Stock Sold", `You sold your stock in '${symbol}' for ${sellValue} BillyBucks at ` +
				`${currentStockInfo.price} ${currentStockInfo.currency}!\n\nYou now have ${user.billyBucks} BillyBucks.`);
		} catch (error) {
			MessagesMethods.ReplyWithErrorEmbed(msg, error);
		}
	...
	*/
	static async Portfolio(msg: Message): Promise<void> {
		try {
			const user = await UserRepo.FindOne(msg.author.id, msg.guild.id);
			...
			if (stocks.length === 0) return MessagesMethods.ReplyWithSuccessEmbed(msg, "Stock Portfolio:", "No active investments!");
	
			let body = "";
			for (let i = 0; i < stocks.length; i++) {
				const stock = stocks[i];
	
				const currentStockInfo = await StocksMethods.GetCurrentStockInfo(stock.tickerSymbol);
			...
			MessagesMethods.ReplyWithSuccessEmbed(msg, "Stock Portfolio:", body);
		} catch (error) {
			MessagesMethods.ReplyWithErrorEmbed(msg, error);
		}
	}
	
	private static async GetCurrentStockInfo(tickerSymbol: string): Promise<{ price: number, currency: string }> {
